...
// JazzKeyboard2Menu.
//=============================================================================
class JazzKeyboard2Menu expands JazzMenu;
...
function SetUpMenu()
{
	...
	AliasNames[3] = "InventoryMenu";
	AliasNames[4] = "UseItem";
	AliasNames[5] = "WeaponNext";
	AliasNames[6] = "WeaponPrev";
	AliasNames[7] = "CellNext";
	AliasNames[8] = "CellPrev";
	AliasNames[9] = "Talk";
	AliasNames[10] = "TeamTalk";
	AliasNames[11] = "Pause";
	...
	for ( i=0; i<255; i++ )
	{
		...
		{	
			...
			Log("Action Configuration Menu) Alias:"$Alias);
			if ( Alias != "" )
			{
				pos = InStr(Alias, " " );
				if ( pos != -1 )
...
function DrawValuesLarge(canvas Canvas, Font RegFont, int Spacing, int StartX, int StartY)
{
	...
	for (i=0; i< MenuLength; i++ )
	{
		...
		{
			SetFontBrightness( Canvas, (i == Selection - 1) );
			Canvas.SetPos(StartX-50, MenuLocations[i] + 4);
			Canvas.DrawText(Caps(MenuValues1[i+1]), false);
			
			if (MenuValues2[i+1] != "")
			Canvas.SetPos(StartX+150, MenuLocations[i] + 4);
			Canvas.DrawText(Caps(MenuValues2[i+1]), false);
		}
		Canvas.DrawColor = Canvas.Default.DrawColor;
	}
}
...
function DrawMenu(canvas Canvas)
{
	...
	// Background
	MenuCenter = Canvas.SizeX * 0.5;
	DrawBackground(Canvas);
	
	Spacing = Clamp(0.06 * Canvas.ClipY, 19, 20);	// Vertical spacing (0.04 = 1/25) // Default Y = 40

	MenuLeftSpace = (0.2 * Canvas.SizeX) + 50;
	...
	{
	MenuLocations[I] = 
	   Min(StartY + Spacing * i , (MenuExistTime * Canvas.ClipY) - Canvas.ClipY - i*1);
	}
	...
	if (Canvas.SizeX>500)
	DrawValuesLarge(Canvas, font'DSFont', Spacing, StartX+MenuLeftSpace, StartY);
	else
	DrawValues(Canvas, font'SSFont', Spacing, StartX+MenuLeftSpace, StartY);
...
defaultproperties
{
     ...
     MenuLength=12
     MenuList(0)="FIRE"
     MenuList(1)="ALTERNATE FIRE"
     MenuList(2)="INVENTORY MENU"
     MenuList(3)="USE ITEM / TALK"
     MenuList(4)="NEXT WEAPON"
     MenuList(5)="PREV WEAPON"
     MenuList(6)="NEXT CELL"
     MenuList(7)="NEXT CELL"
     MenuList(8)="TALK"
     MenuList(9)="TEAM TALK"
     MenuList(10)="PAUSE"
     MenuList(11)="RESET TO DEFAULT"
     MenuTitle="ACTION CONTROLS"
     Menutype=4.000000
}
