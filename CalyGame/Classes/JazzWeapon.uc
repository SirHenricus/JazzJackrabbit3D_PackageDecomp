...
var()   bool			NoNewDisplay;			// New Item Display

var()	int				WeaponSwitchNumber;		// Weapon switch number
...
auto state Pickup
{
	event Tick (float DeltaTime) // Bobbing!
	{
		local vector NewLocation;
		if (Bobbing == True && Physics != PHYS_Falling) // Patch change - do the bobbing effect!!!
		{
			BobDir += DeltaTime*40000;
			Bob.Z = sin(BobDir * (pi/32768))*7;
			NewLocation = OrigLoc+Bob;
			SetLocation(NewLocation);
			bCollideWorld = False;
		}
		else if (Bobbing == True)
		{ OrigLoc = location; }	
	}
...
function ExtractCellList ()
{
	...
	while (Inv != None)
	{
		...
		{
			if (JazzWeaponCell(Inv).WeaponCapable[JazzWeapon(Pawn(Owner).Weapon).WeaponTypeNumber]==0)
			{
			//Log("ValidCell) False");
			ValidCell = false;
			}
