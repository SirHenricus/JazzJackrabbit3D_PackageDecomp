...
function bool AddBot()
{
	...
	// HACK  -  Randomize bot skin color for non team games
	
	NewSkinNum = 34*Frand();
	
	// Set bot's skin
	if ( (NewSkinNum >= 0) && (NewSkinNum < 35) )
	{
		NewSkin = texture(DynamicLoadObject(BotConfig.RandomBotSkins[NewSkinNum], class'Texture'));
		if ( NewSkin != None )
		{
			NewBot.Skin = NewSkin;
...
defaultproperties
{
     ...
     HUDType=Class'CalyGame.JazzBattleHUD'
     MapPrefix="JMP"
}
