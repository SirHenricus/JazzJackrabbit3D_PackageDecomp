...
// HUD 3 Display
var() texture		Hud3Main;
var() texture		Hud3GunNorm;
var() texture		Hud3HealthFull;
var() texture		Hud3HealthEmpty;	
...
var float				ConvWindowYSizeTarget;	// Y Size Target of conversation window display
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Purchase Message Subsystem 											HUD SUBSYSTEM
///////////////////////////////////////////////////////////////////////////////////////////////
//
var		GlowingPurchaseObject	PurchaseItem;
var		float					PurchaseLife;

///////////////////////////////////////////////////////////////////////////////////////////////
// Purchase Message Subsystem 											HUD SUBSYSTEM
///////////////////////////////////////////////////////////////////////////////////////////////
//
var		Inventory		NewItemItem;
var		float			NewItemLife;
var		string			NewItemName;
...
simulated function DrawCrossHair( canvas Canvas, int StartX, int StartY )
{
	...
	Canvas.SetPos(StartX-8, StartY+16 );
	Canvas.Style = 2;
	if (JazzPlayer(Owner).TargetIcon != None)
	{ Canvas.DrawIcon(JazzPlayer(Owner).TargetIcon, 1.0); }
	/*if		(Crosshair==0) 	Canvas.DrawIcon(Texture'target1', 1.0);
	else if (Crosshair==1) 	Canvas.DrawIcon(Texture'target2', 1.0);	
	else if (Crosshair==2) 	Canvas.DrawIcon(Texture'target3', 1.0);
	else if (Crosshair==3) 	Canvas.DrawIcon(Texture'target4', 1.0);*/	
	Canvas.Style = 1;	
}
...
function DrawHudKinetic(canvas Canvas)
{
	...
	//MiscObjectDisplay	(Canvas,100,150,2,0.14);		// 0.14 * ScreenWidth
		...
		//ComponentX = Canvas.SizeX-40;
		//ComponentY = 4;
		DancingCarrot	(Canvas,ComponentX,ComponentY);
		
		if (ProcessComponent(1,Canvas.SizeX+10,-50,Canvas.SizeX-60,34))
		...
		{
		DancingMessage	(Canvas,DancingMessageText,Canvas.SizeX/2,Canvas.SizeY*0.1);
		if (VersionMessageText != "")
		DancingMessage	(Canvas,VersionMessageText,Canvas.SizeX/2,Canvas.SizeY*0.6);
		}

		DrawWeapon		(Canvas,5,Canvas.SizeY);

/*		if (ProcessComponent(7,Canvas.SizeX+10,Canvas.SizeY+10,Canvas.SizeX*0.9,Canvas.SizeY*0.9))*/
		ComponentX = Canvas.SizeX-128;
		ComponentY = Canvas.SizeY-128;
		DrawLives		(Canvas,ComponentX,ComponentY);
		
		// Health Carrot
		//SidewaysHealthDisplay(Canvas,Canvas.SizeX*0.5,Canvas.SizeY);	// Just taken out - Old sideways health carrot display
		//if (ProcessComponent(5,-100,-50,Canvas.SizeX*0.03,Canvas.SizeY*0.03))
		//DeanHudTypeA	(Canvas,ComponentX,ComponentY);

		// draw the targeting bubble
		Canvas.DrawActor(JazzPlayer(Owner).Targeting,false);	
...
function DrawWeapon(canvas Canvas, float XPos, float YPos)
{
	...
	local float WeapRechargeTime,ManaPct;
	...
	// HUD 3 Health Display
	Canvas.SetPos(XPos+280*Scale,YPos-73*Scale);
	Canvas.DrawIcon(HUD3HealthEmpty,Scale);
	...
	Canvas.SetPos(XPos+280*Scale,YPos-73*Scale);
	Draw = 125*HealthPct+58;
	Canvas.DrawTile(HUD3HealthFull,Draw*Scale,HUD3HealthFull.VSize*Scale,0,0,Draw,HUD3HealthFull.VSize);
	
	// HUD 3 Main Display
	Canvas.SetPos(XPos,YPos-132*Scale);
	Canvas.DrawIcon(HUD3Main,Scale);

	// HUD 3 Weapon Display
	Canvas.SetPos(XPos+52*Scale,YPos-76*Scale);
	if ((JazzPlayer(Owner) != None) && (JazzPlayer(Owner).Weapon != None))
	{
		WeapIcon = JazzPlayer(Owner).Weapon.Icon;
		Canvas.DrawIcon(WeapIcon,Scale/2);
	}
	
	// HUD 3 Weapon Charge	
	if ((JazzPlayer(Owner) != None) && (JazzPlayer(Owner).Weapon != None))
	{
	ManaPct = JazzPlayer(Owner).ManaAmmo/JazzPlayer(Owner).ManaAmmoMax;
	Canvas.SetPos(XPos,YPos-132*Scale);
	Canvas.DrawTile(HUD3GunNorm,142*Scale*ManaPct,HUD3GunNorm.VSize*Scale, 0, 0, 142*ManaPct, HUD3GunNorm.VSize );
	}
	
	// HUD 3 Dreamcell Display
	Canvas.SetPos(XPos+131*Scale,YPos-71*Scale);
...
/*	Canvas.Style = 2;
	...
	// Mana Ammo Remaining
	Canvas.Style = 3;
	Canvas.SetPos(XPos,YPos-20);
	
	if ((JazzPlayer(Owner) != None) && (JazzPlayer(Owner).Weapon != None))
	{
		Canvas.Font = font'DSFont';
		Canvas.DrawText(int(JazzPlayer(Owner).ManaAmmo/10));
	}
...
/*	Canvas.Style = 2;
	...
	Canvas.SetPos(XPos+Scale*14,YPos);
...
// The location passed to this function is intended to be the bottom center of the carrot image.
//
/*function SidewaysHealthDisplay( canvas Canvas, float XPos, float YPos )
{
	local float Scale,X,Y,Draw,HealthPct;
	local texture T;
	...
	/*T = texture'CARROTCE';
	X = XPos-T.USize*Scale/2;
	Y = YPos-T.VSize*1.2*Scale;
	Canvas.SetPos(X,Y);
	Canvas.DrawIcon(T,Scale);*/

	// Carrot Top
	T = texture'CAROTTOP';
	Canvas.SetPos(XPos-(T.USize-4)*Scale,YPos-56*Scale);
	Canvas.DrawIcon(T,Scale);
	
	T = texture'CAROT2';
	Canvas.SetPos(XPos,YPos-64*Scale);
	...
	Canvas.SetPos(XPos,YPos-64*Scale);
	Draw = 98*HealthPct;
	Canvas.DrawTile(T,Draw*Scale,T.VSize*Scale,0,0,Draw,T.VSize);
}*/
...
function DancingCarrot(canvas Canvas,float XPos, float YPos)
{
	...
	XPos -= (CarrotSize-1) * 200;

	DisplayMesh( Canvas, mesh'Coin', JCoin_01, XPos, YPos+54, 0.1, LifeTime*10000 );
...
/*	Canvas.SetPos(XPos,YPos+(10*Sin((DancingCarrotLife/360)*Pi*2)));
	...
	{
		Canvas.SetPos(XPos+(20*Ch),YPos+(10*Sin(((DancingCarrotLife-40+(Ch*40))/360)*Pi*2))+(8*(CarrotSize-1)));
...
function DancingScore(canvas Canvas,float XPos, float YPos)
{
	...
	for (Ch=0; Ch<=Len(CarrotNum); Ch++)
	{
		Canvas.SetPos(XPos+18*CarrotSize+(20*Ch),YPos+(5*Sin(((DancingCarrotLife-40+(Ch*40))/360)*Pi*2))+(8*(CarrotSize-1)));
		Canvas.DrawText(Mid(CarrotNum,Ch,1),false);
...
function DancingMessage(canvas Canvas,string Message,float XPos, float YPos)
{
	...
	if (DancingMessageDuration>0)
	{
		...
		if (Len(Message)*26>Canvas.SizeX)
		{
		...
		Canvas.DrawText(Mid(Message,Ch,1),false);	// CAPS crashes now for some reason.
		X=Canvas.CurX;
		}
	}
}
...
event Tick(float DeltaTime)
{
	...
	// Patch Change - Hide the Hud if desired
	//bHidden = Hud(Owner).GameHud;
	...
	// Purchase System Tick
	PurchaseTick(DeltaTime);
}
...
event UpdateInventoryTick ( float DeltaTime )
{
	...
	// (Roundabout way since I can't do actual code changes but should work perfectly)
		...
		// Patch change - since most players would probably use a mouse+keyboard setting,
		// we want them able to easily change the groups by pressing A or D
		if ( (JazzPlayer(Owner).aStrafe < 0) || (JazzPlayer(Owner).bEdgeLeft) )
		{
			if (LeftDelay<=0)
			{
		...
		// Patch change - since most players would probably use a mouse+keyboard setting,
		// we want them able to easily change the groups by pressing A or D
		if ( (JazzPlayer(Owner).aStrafe > 0) || (JazzPlayer(Owner).bEdgeRight) )
		{
			if (RightDelay<=0)
			{
			InventoryItemGroupNum++;
...
event UpdateInventoryDisplay( canvas Canvas )
{
	...
	// Patch change - draw the black box behind the item objects!
	/*if (InventoryDisplayActive)
	{
		InvX += ( Canvas.SizeX*0.4 - InvX)*0.1;
		
		Canvas.Style = 4;
		Canvas.SetPos(Canvas.SizeX-InvX,0);
		Canvas.DrawPattern( BoxPat1,Canvas.SizeX-InvX+Canvas.SizeX*0.2, Canvas.SizeY, 1 );
	}
	else
	{
		InvX -= InvX*0.1;
...
//	T = WeapFlar;
	...
	/*if ( InventoryItems[J].Icon == JazzArt.Interface.Hud3Dreamcell640norm )
	{ T = WNormal; }//InventoryItems[J].Icon; }
	if ( InventoryItems[J].Icon == JazzArt.Interface.Hud3Dreamcell640fire )
	{ T = WeapFire; }//InventoryItems[J].Icon; }
	if ( InventoryItems[J].Icon == Hud3Dreamcell640ice )
	{ T = WeapIce; }//InventoryItems[J].Icon; }*/
	
	
	X = Canvas.SizeX*0.95 + (1-cos(abs(K)/float(InventoryItemListNum+1)))*Canvas.SizeX*0.3;
	Y = Canvas.ClipY*(K*Spacing+0.5)-T.VSize*TempScale/2;
	
	// Patch change - We don't need to draw multiple black boxes - just one will do.
	//Canvas.Style = 4;
	//Canvas.SetPos(X,Y);
...
//	Canvas.DrawTile( BoxPat1, Canvas.SizeX, (T.VSize-1)*TempScale, 0, 0, 8, 8 );
	...
	DrawInventoryItem(	X, 	Canvas.ClipY*(K*Spacing+0.5), // Canvas.SizeX*0.6
...
function UpdateTutorialDisplay( canvas Canvas )
{
	...
	Canvas.SetPos(Canvas.SizeX*0.1,Canvas.SizeY*0.4);
	...
	Canvas.SetPos(Canvas.SizeX*0.2,Canvas.SizeY*0.4);
	if (CapsOnly==true)
	...
	Canvas.OrgY = Canvas.SizeY*0.45;
	Canvas.ClipX = Canvas.SizeX*0.74;
	Canvas.ClipY = Canvas.SizeY*0.72;
...
// Display mesh.
//
function DisplayMesh( canvas Canvas, mesh MeshToDisplay, texture MeshSkin, float XPos, float YPos, float Scale, optional float YawAdd, optional float YawRot )
{
	...
	DrawRot = JazzPlayer(Owner).MyCameraRotation;
	DrawRot.Pitch = -JazzPlayer(Owner).MyCameraRotation.Pitch;
	DrawOffset = (DrawOffset >> JazzPlayer(Owner).MyCameraRotation);
	DrawLoc = JazzPlayer(Owner).MyCameraLocation /*+ Pawn(Owner).EyeHeight * vect(0,0,1)*/;
	...
	YawRot += 1;
	if (YawRot > 256) { YawRot = 0; }

	//Log("DrawMesh) "$Mesh$" "$Scale$" "$DrawLoc$" "$JazzPlayer(Owner).MyCameraLocation);
	
	SetLocation(DrawLoc + DrawOffset);
	// Patch change - Fixed item spinning - their pitch no longer changes when the camera looks up or down.
	//NewRot.Yaw = DrawRot.Yaw + YawRot;
	//NewRot.Roll = DrawRot.Roll;
	//NewRot.Pitch = DrawRot.Pitch;
	NewRot = DrawRot;
	NewRot.Yaw += YawAdd;
	//NewRot.Pitch = 0;
	SetRotation(NewRot);
...
function InitiateConversation (JazzConversation NewConversation,pawn EventInstigator,optional bool LargeWindow)
{
	...
	if (!LargeWindow)
	{
		ConvWindowYSizeTarget = 0.3; // 0.2 // Patch change - Set the target size
	}
	else
	{
		ConvWindowYSizeTarget  = 0.4; // Patch change - Set the target size
	}
...
event UpdateConversationDisplay (canvas Canvas)
{
	...
	local int MaxChar,CurChar,X,Y;
	
	local float MaxX,TextX,TextY,CurX,CurY,TX,TY,Scale;
	local int CharInLine,LinePos;
	local bool WordOk;

	if (CurrentConversation != None)
	{
		//ConvWindowYSize += ( ConvWindowYSizeTarget - ConvWindowYSize ) * 0.05; // Patch change - the window smoothly scales up
		ConvWindowYSize += ( ConvWindowYSizeTarget - ConvWindowYSize ) * 0.05; // Patch change - the window smoothly scales up
		...
		T = HudMisc;
		Canvas.SetPos(Canvas.SizeX*0.1,Canvas.SizeY*(0.7-ConvWindowYSize));
		Canvas.DrawTile(T,Canvas.SizeX*0.8,Canvas.SizeY*ConvWindowYSize,0,0,T.USize,T.VSize);
		// Patch change - the Conversation box is stretched out now proportionally in order to not only hold longer texts but also to just look better.
		//Canvas.DrawTile(T,Canvas.SizeX*0.8,Canvas.SizeY*0.3,0,0,T.USize,T.VSize);
		...
		{
			Canvas.Style = 2;
			//ConvWindowYSizeTarget = ?; // Patch change - FIXME - Need to find how many lines are there, so the
			// window streches out dynamically, just as much as it needs to fit the text in!
		}
		else
		{
			Canvas.Style = 3;
			ConvWindowYSizeTarget = 0;
		}
		MaxChar = Len(ConvText);	// Max chars in string
		CurChar = (ConvTime / 0.08);	// Chars to draw
		if (CurChar>MaxChar) 
		{
			CurChar=MaxChar;
			ConvEndLine = true;
		...
		CurY = Canvas.SizeY*(0.72-ConvWindowYSize);
		LinePos = 0;
		CharInLine = (Canvas.SizeX*0.84 - Canvas.SizeX*0.18) / TextX + 5;
		...
		for (x=0; x<CurChar; x++)
		{
			...
			CurX += TX;
			LinePos++;
			if (CurX > Canvas.SizeX*0.84)	// At Next line
			{
				CurX = Canvas.SizeX*0.18;
				CurY += TextY;
				LinePos = 0;
			...
			if (Mid(ConvText,x,1)==" ")
			{
				...
				for (Y=X+1; (Y<=X+(CharInLine-LinePos)+2) && (WordOk==false); Y++)
				{
					if (Y>=MaxChar)
						WordOk = true;
					else
				...
				if (WordOk == false)		// Force next line
				{
					CurX = Canvas.SizeX*0.18;
					CurY += TextY;
					LinePos = 0;
		...
		{
			T = ConversationCon;
			Canvas.SetPos(Canvas.SizeX*0.78,Canvas.SizeY*0.65);
			Canvas.Style = 3;
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Purchase Item Subsystem 												HUD SUBSYSTEM
///////////////////////////////////////////////////////////////////////////////////////////////
//
//
//
...
//
//
function StartPurchase( GlowingPurchaseObject Item )
{
	local int i;

	JazzPlayer(Owner).bShowPurchase = true;
	//JazzPlayer(Owner).Player.Console.GotoState('Menuing');
	//if( Level.Netmode == NM_Standalone )
		//JazzPlayer(Owner).SetPause(true);

	// Store item to purchase	
	PurchaseItem = Item;
...
function PurchaseTick ( float DeltaTime )
{
	PurchaseLife += DeltaTime;
	
	// Check if purchase display active (item selected)
...
function UpdatePurchaseDisplay ( canvas Canvas )
{
	local string CarrotNum;
	...
	if (PurchaseItem != None)
	{
		// Makre sure coins carried are being displayed
		//UpdateCarrots();
		
		// Display Name of Item
		Canvas.SetPos(
			Canvas.SizeX * 0.9 - Len("PURCHASE")*8,
			Canvas.SizeY * 0.35);
		
		Canvas.Font = font'DSFont';
		Canvas.DrawText("PURCHASE");
		
		// Doo
		Canvas.SetPos(
			Canvas.SizeX * 0.9 - Len(PurchaseItem.NameOfItem)*8,
			Canvas.SizeY * 0.35-18);
		
		Canvas.Font = font'DSFont';
		Canvas.DrawText(Caps(PurchaseItem.NameOfItem));
		
		// Display Item spinning underneath name
		// Patch change - fixed the mesh location
		//DisplayMesh(Canvas,PurchaseItem.Mesh,PurchaseItem.Texture,
			//Canvas.SizeX * 0.9,Canvas.SizeY * 0.4, 0.5, PurchaseLife*20*360);
			
		DisplayMesh(Canvas,PurchaseItem.Mesh,PurchaseItem.Texture,
			Canvas.SizeX * 0.8,Canvas.SizeY * 0.7, 0.5, PurchaseLife*20*360);
		
// 208 225 193 19
//
// 198 164 124 103
		
		// Display coin spinning underneath item (down to the left a bit)
		DisplayMesh(Canvas,mesh'coin',texture'jcoin_01',
			Canvas.SizeX * 0.85,Canvas.SizeY * 0.5, 0.05, PurchaseLife*80*360);
		...
		Canvas.SetPos(
			...
			Canvas.SizeY * 0.5);
		Canvas.Font = font'DSFont';
		Canvas.DrawText(PurchaseItem.CostInCoins);
	}
}

function PurchaseEnd ()
{
	// Remove player from paused state
	PurchaseItem=None;
	JazzPlayer(Owner).bShowPurchase = false;
	//JazzPlayer(Owner).Player.Console.GotoState('');
	//if( Level.Netmode == NM_Standalone )
		//JazzPlayer(Owner).SetPause(false);
}
...
function DoPurchase ()
{
	PurchaseItem.DoPurchase(Owner);
}

///////////////////////////////////////////////////////////////////////////////////////////////
// New Item Display Subsystem											HUD SUBSYSTEM
///////////////////////////////////////////////////////////////////////////////////////////////
//
//
//
...
//
//
function UpdateNewItemDisplay ( canvas Canvas )
{
	// Display active?
	if (NewItemItem == None)
	return;
	...
	{
		// Start Fading out
		Canvas.DrawColor.R = 255*(1-(NewItemLife-10));
		Canvas.DrawColor.G = 255*(1-(NewItemLife-10));
		Canvas.DrawColor.B = 255*(1-(NewItemLife-10));
		Canvas.Style = 3;
	...
	{
		// Start Fading in
		Canvas.DrawColor.R = 255*NewItemLife;
		Canvas.DrawColor.G = 255*NewItemLife;
		Canvas.DrawColor.B = 255*NewItemLife;
		Canvas.Style = 3;
		Style = STY_Translucent;
	}
	else
	...
	// Display Mesh
	DisplayMesh(Canvas,NewItemItem.PickupViewMesh,NewItemItem.Skin,
		Canvas.SizeX * 0.9,Canvas.SizeY * 0.9, NewItemItem.DrawScale/2, NewItemLife*120*360);
	Style = STY_Normal;
	
	//Log("NewItemDisplay) "$NewItemItem.PickupViewMesh$" "$NewItemItem.Skin);
	//Log("NewItemDisplay) "$NewItemItem.PickupMessage$" "$NewItemItem.ItemName);
	
	Canvas.SetPos(Canvas.SizeX*0.9-Len(NewItemName)*8,Canvas.SizeY*0.9);
	Canvas.Font = Font'DSFont';
	Canvas.DrawText(Caps(NewItemName));
}

function NewItemTick ( float DeltaTime )
{
	NewItemLife += DeltaTime;
}

function NewItemStart ( Inventory NewItem, string ItemName )
{
	NewItemItem = NewItem;
	NewItemLife = 0;
...
defaultproperties
{
     ...
     Hud3Main=Texture'JazzArt.Interface.Hud3'
     Hud3GunNorm=Texture'JazzArt.Interface.Hud3Meter'
     Hud3HealthFull=Texture'JazzArt.Interface.Hud3LifeFull'
     Hud3HealthEmpty=Texture'JazzArt.Interface.Hud3LifeEmpty'
     ...
     MenuBack=Texture'JazzArt.Interface.MenuBack'
     JoyBut1=Texture'JazzArt.Control.JoyBut1'
     HudItem1=Texture'JazzArt.Interface.HudItem1'
     HudItem2=Texture'JazzArt.Interface.HudItem2'
     NewHud=Texture'JazzArt.Interface.NewHud'
     NewHudCE=Texture'JazzArt.Interface.NewHudCE'
     NewHudC=Texture'JazzArt.Interface.NewHudC'
     HudMisc=Texture'JazzArt.Interface.HudMisc'
     HudMiscT=Texture'JazzArt.Interface.HudMiscT'
     WeapRedy=Texture'JazzArt.Interface.WeapRedy'
     Jcoin_01=Texture'JazzObjectoids.Skins.Jcoin_01'
     BoxPat1=Texture'JazzArt.Interface.BoxPat1'
     JazzGrp2=Texture'JazzArt.Interface.JazzGrp2'
     EventN=Texture'JazzArt.Events.EventN'
     ConversationCon=Texture'JazzArt.Interface.ConversationCon'
     LivesFace1=Texture'JazzArt.Interface.facesmall'
