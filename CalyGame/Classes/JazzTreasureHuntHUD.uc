...
function MiscObjectDisplayBar(canvas Canvas)
{
	...
	// Display Time Remaining
	Temp = (BattleMode(Level.Game).TimeLimit*60) - Level.TimeSeconds;
	if (Temp<0) Temp=0;
	
	Min = (Temp)/60;
	Sec = (Temp) - (Min*60);
	Temp =(Temp) - (Min*60) - Sec;
	PSec = Temp * 100;
...
defaultproperties
{
     JazzKey=Texture'JazzArt.Items.JazzKey'
     Clock=Texture'JazzArt.Interface.Clock'
     JazzGem=Texture'JazzArt.Interface.JazzGem'
}
