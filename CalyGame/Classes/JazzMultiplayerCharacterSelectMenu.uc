...
{ 
	...
	if ( NewPlayerClass != None )
	{
		...
		// Patch change - let's use a different animation for this!
		//NewPlayerClass.Default.AnimSequence,1.0);
		//if (JazzPlayer(PlayerOwner).bBehindView)
		//{ LoopAnim('JazzRun',1); }
		//else
		LoopAnim('JazzRun',0.5);
		Log("Multiplayer Character Select Animation) "$NewPlayerClass.Default.AnimSequence$" "$Mesh$" "$AnimSequence$" "$AnimRate);
	}
...
function DrawMenu (canvas Canvas)
{
	...
	// Patch change - Yeah, this is VERY hacky... probably. But hey... it works!
	JazzIntroHUD(Owner).ChSelDummy.texture = Texture(DynamicLoadObject(PreferredSkin, class'Texture'));
...
/*	MenuSelections[0] = PlayerClassName[ClassSelect-1];
	...
	//PlayerOwner.ViewRotation.Pitch = 0;
	//PlayerOwner.ViewRotation.Roll = 0;
//	if (JazzPlayer(PlayerOwner).MyCameraRotation != vect(0,0,0))
	if (JazzPlayer(PlayerOwner).bBehindView)
	{
		Log("CharSelect) "$JazzPlayer(PlayerOwner).MyCameraRotation$" "$PlayerOwner.bBehindView);
	...
	{
		Log("CharSelect) "$JazzPlayer(PlayerOwner).MyCameraRotation$" "$PlayerOwner.bBehindView);
		DrawRot = JazzPlayer(PlayerOwner).ViewRotation;
		DrawOffset = (vect(0,-25,0)) >> JazzPlayer(PlayerOwner).ViewRotation;
		DrawLoc = JazzPlayer(PlayerOwner).Location+ PlayerOwner.EyeHeight * vect(0,0,1);
	}
	...
	NewRot.Pitch = 2000;
	SetRotation(NewRot);
	if (JazzPlayer(PlayerOwner).bBehindView) // Patch change - The main menu has it's own awesome character selection thingy!
