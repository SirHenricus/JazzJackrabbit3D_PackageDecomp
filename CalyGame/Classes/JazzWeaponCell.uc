...
var()	bool			Flaming;				// Patch change - Used for emmiting the fire particles from the Fire Dreamcell
...
auto state Pickup
{
	event Tick (float DeltaTime) // Bobbing!
	{
		...
		// Patch change - The Dreamcells can now emmit special particles to add more detail on them!
		local FireParticle Part;
		local vector PartLocation;
	
		if (Flaming == True) // Emmit the fire particles!
		{
			PTime += DeltaTime*40;
			if (PTime > 1)
			{
				PartLocation = VRand()*10;
				PartLocation.Z = Location.Z-20;
...
function Projectile ProjectileFire(class<projectile> ProjClass, float ProjSpeed, bool bWarn, optional rotator RotateLaunch )
{
	...
	AdjustedAim = JazzPlayer(Owner).MyCameraRotation;//pawn(owner).AdjustAim(ProjSpeed, Start, AimError, True, (3.5*FRand()-1<1));
...
function Fire( float Value )	// Change DoFire instead
{
	...
	//local vector  TargLoc,FacingVector;
	...
	if (MainReadyTime<=0)		// Alt fire ready?
	{
		...
		// Patch change - most of the code here is altered in order to target the weapon to the crosshair position;
		
		
		SetPhysics(PHYS_Rotating);
		newRotation = Pawn(owner).ViewRotation;
		
		/*TargLoc = JazzPlayer(Owner).Targeting.Location;
...
function AltFire( float Value )	// Change DoAltFire
{
	if (AltReadyTime<=0)		// Main fire ready?
	{
		...
		break;
		case 2: // Missile Launcher
			GotoState('AltFireStateJazz','Missile');
		break;
		case 5: // Shiv
			GotoState('FireStateJazz','Gizmo');
		break;
...
//
function ChargeStart( name LChargeState, class LChargeEffect, bool bAltFire, 
		float LChargeEffectSize, optional float ScalePeriod, optional float ScaleTarget,
		optional float StartVel, optional float EndVel, 
		optional float StartVolume, optional float EndVolume )
{
	if (bAltFire)
	{
		AltCharging = true;
	...
	if (JazzChargingEffects(ChargeEffectActor) != None)
	{
		// Jazz Charging Effect - Calypso
	...
	if (JazzChargeEffect(ChargeEffectActor) != None)
	{
		//if (JazzChargeEffect(ChargeEffectActor).bActive==false)
		// Jazz Charging Effect - Devon
		JazzChargeEffect(ChargeEffectActor).Activate(
			Owner,
			LChargeEffectSize,
			JazzChargeEffect(ChargeEffectActor).ActiveParticles,
			ScalePeriod,
			ScaleTarget,
			StartVel,
			EndVel,
...
function ChargeEnd ()
{
	...
	JazzChargeEffect(ChargeEffectActor).Deactivate();
...
function bool CheckAvailableAmmo( float Amount )
{
	if (JazzPlayer(Owner) != None)
	{
	// Patch change - The ammo check is now fixed ( I'm not sure if I broke it or it was broken before )
	if (JazzPlayer(Owner).ManaAmmo > Amount
	 && (JazzPlayer(Owner).IsInState('PlayerWalking') || JazzPlayer(Owner).IsInState('PlayerSwimming')))
	{ return(true); }
	else
	{ return(false); }
...
//	return(JazzPlayer(Owner).ManaAmmo >= Amount);
	//else
	//return(true);		// TODO: Hack in ammo amounts
	}
}
...
{
	// Patch change - The ammo usage is now fixed ( I'm not sure if I broke it or it was broken before )
	if (JazzPlayer(Owner) != None)
	//JazzPlayer(Owner).ManaAmmo = JazzPlayer(Owner).ManaAmmoMax -= Amount;
	{
		if (JazzPlayer(Owner).ManaAmmo > Amount)
...
//Log("StopSound");
//Owner.PlaySound(sound'FireShot',,0);
}
...
defaultproperties
{
     ...
     PickupViewMesh=LodMesh'JazzObjectoids.wcell'
