...
function NavigationPoint FindPlayerStart(Pawn Player, optional byte InTeam, optional string incomingName)
{
	...
	for ( N=Level.NavigationPointList; N!=None; N=N.nextNavigationPoint )
		if ( N.IsA('PlayerStart') 
			&& (!bSpawnInTeamArea || (InTeam == PlayerStart(N).TeamNumber)) )
		{
			if (num<4)
			...
			for (i=0;i<num;i++)
			{
				...
				if ( OtherPlayer.Region.Zone == Candidate[i].Region.Zone )
				{
					if (NextDist < CollisionRadius + CollisionHeight)
						Score[i] -= 1000000.0;
					else if ( (NextDist < 1400) && (InTeam != OtherPlayer.PlayerReplicationInfo.Team) && OtherPlayer.LineOfSightTo(Candidate[i]) )
						Score[i] -= 10000.0;
				}
			}
...
function bool AddBot()
{
	...
	else if ( !ChangeTeam(NewBot, DesiredTeam) )
	{
		ChangeTeam(NewBot, NextBotTeam);
		NextBotTeam++;
		if ( NextBotTeam >= MaxTeams )
...
defaultproperties
{
     ...
     HUDType=Class'CalyGame.JazzTeamHUD'
