...
function GainLeader(float DrawScaleChange, float JumpZChange, bool bTLand)
{
	...
	log("JazzThinker) "$self $ " Gained leader");
...
function LoseFollower(pawn Other)
{
	if(bFollow)
	{
		log("JazzThinker)"$self $ " was following and tried to lose " $ other $ " as a follower");
	}
	else if(!bFollowed)
	{
		log("JazzThinker)"$self $ " was not being followed and tried to lose " $ other $ " as a follower");
	}
	else
	{
		Followers--;
...
function ChangeLeader( pawn NewLeader)
{
	if(NewLeader == None)
	{
		log("JazzThinker)"$self $ " was told to change leader to none");
	}
	else
	{
...
function GiveFollower( pawn Other)
{
	...
	if(other == None)
	{
		log("JazzThinker)"$self $ " tried to give followers to no one");
		return;
	}
	...
	if(Followers != 0)
	{
		log("JazzThinker)"$self $ "Gave followers away and ended up with " $ Followers);
	}
}
...
ignores SeePlayer, EnemyNotVisible, HearNoise, Died, Bump, Trigger, HitWall, HeadZoneChange, FootZoneChange, ZoneChange, Falling, WarnTarget, LongFall, PainTimer;
	...
	{
		if ( Health <= 0 )
			log("JazzThinker)"$self$" health still <0");
	}
