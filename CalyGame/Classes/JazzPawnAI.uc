...
var(JAttack) float  ProjRecharge;					// Recharge time for projectile attack
...
var	bool	ReadyForThreat;			// Threat sound - make one?

var float 	EnemyFound;		// Time until next EnemyFound check
...
function PreBeginPlay()
{
	...
	// We want to do a threat sound only once
	ReadyForThreat = true;
}
...
function DamageOpponent(rotator Rotate, float Dist, float Damage, name DamageType, float Momentum)
{
	...
	if (Pawn(Other) != None)
	{
		...
		// Patch change - when Jazz is hurt, he usually gets kicked away. Lets make jazz display the falling
		// animation whenever that happens.
		if (other == Jazzplayer(other))
		{ Jazzplayer(other).setphysics(Phys_Falling); 
		Jazzplayer(other).PlayFallingDown();//Jazzplayer(other).TweenToFallingDown(0.1); }
		Jazzplayer(other).TweenToFallingDown(0.1); }
	}
	else
	{
		Rotate.Pitch += 5000;
						...
						Vector momentum, name damageType)
{
	...
	if (IsNPC != true) // Patch change - This only works if the Pawn is not an NPC
	{
		if ((!StateBasedInvulnerability) && (InvulnerabilityTime<=0))
		{
			...
			if (DamageHitBack == true)
			{ Super.TakeDamage(Damage, instigatedBy, hitlocation, momentum, damageType); }
			else
			{ Super.TakeDamage(Damage, instigatedBy, hitlocation, velocity, damageType); }
		}
	}
}
...
state Decision
{
	...
	function CheckTargetVisibility()
	{
		...
		if (EnemyFound<=0)	// Enemy needs to be checked again?
		if (JazzPlayer(Enemy) != None)
		{
			if (JazzPlayer(Enemy).Visibility<128)
			if (fRand()*128>JazzPlayer(Enemy).Visibility)
			Enemy=None;		// Lost enemy
			else
			EnemyFound=5;
		}
	}
...
function DoProjectileAttack(optional bool CheckAccuracy)
{
	...
	// Skip if not recharged yet
	if (ProjRecharge > 0)
	return;

	// Recharge time for projectiles.  Just a simplistic hack for now.
	//
	ProjRecharge = 2 - TriggerHappy * 0.4 + FRand()*Default.ProjRecharge;
...
Begin:
	if (LineOfSightTo(Enemy))
	{
		...
		if (ReadyForThreat)
		{
			ReadyForThreat = false;
			VoicePackActor.ThreatSound(Self);			
		}
