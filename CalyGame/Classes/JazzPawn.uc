...
var(JazzPawn) bool			IsNPC;					// NPC's have a few special characteristics, like not being able to die.
...
var(Display) bool 			BlobShadow; 			// Patch change - All characters can have blob shadows now
var(Display) float			BlobShadowBlackness;	// 1 - light shadow; 2 - medium shadow; 3 - dark shadow
var(Display) float			BlobShadowSize;			// This is yet to be implemented
...
var(JDeath) name			DeathTriggerTagOnce;	// Trigger actors matching this when dead - not redone as a Unique Enemy.
var(JDeath) bool			UniqueEnemy;			// Unique enemy - Set Death Trigger Tag
//var(JDeath) sound			DeathSound;
var(JDeath) class 			JumpedOnDeathEffect;
var(JDeath) class 			TouchedDeathEffect;		// This actor killed by touching
...
var(JDamage)	bool	DamageHitBack;			// Patch change - this is used mainly for the lizards, because they shouldn't stop after they get hit
...
////////////////////////////////////////////////////////////////////////////////////////////
// Stupid Patch crap - for random uses ( I am not too familiar with this engine, so... sorry about that )
////////////////////////////////////////////////////////////////////////////////////////////

// This is used to spawn the blob shadow below the characters
...
function BeginPlay ()
{
	...
	if (BaseJazzPawnWeapon != None)
	{
		...
		// Reset Weapon Selected
		if (Weapon != None)
		JazzWeapon(Weapon).ChangeCell(0);		
	}
	
	// Patch change - This used for the shadow
	if (BlobShadow == True)
	{
		if (isNPC == True)
		{
...
function Tick( float DeltaTime )
{
	...
	if (BurnTime>0)
	{	
		NextBurnDamage -= DeltaTime;
		if (NextBurnDamage<=0)
		{
			TakeDamage(5,self,self.Location,vect(0,0,0),'Fire');
			NextBurnDamage=6;
		}
...
/*function NewTemporaryEffect( JazzEffect E )
{
...
function Burn(optional int Level)
{
	...
	//Log("Burn) "$BurnTime$" "$bBurnable);
	
	if ((BurnTime<=0) && (bBurnable==true))
	{
		// Last 15 seconds
		BurnTime = 15;
		NextBurnDamage = 3;
...
{
	if (IsNPC != true) // Patch change - This only works if the Pawn is not an NPC
	{
		if(bPetrify)
		{
			switch(Level)
...
function DetermineTypeOfBump( pawn Other )
{
	...
	if (IsNPC != true) // Patch change - This only works if the Pawn is not an NPC
	{
		...
		if  (
			...
			&&
			(Other.Velocity.Z < 0)								// Is the other actor falling? // Patch Change
			)
		{
			HitByJumping(Other);
		}
...
function HitByJumping( pawn Other )
{
	...
	if ((Other != None) && (!IsNPC)) // Patch change - This only works if the Pawn is not an NPC
	{
		if (JumpOnTakeDamage)
		{
			...
			//
			if (health <= JumpOnDamage)
			{
				JumpedOnDied(Other,'JumpedOn',Other.Location);
			}
			else
			{
				TakeDamage(JumpOnDamage,Other,Other.Location,vect(0,0,0),'JumpedOn');
				PlayHit(JumpOnDamage,vect(0,0,0),'JumpedOn',vect(0,0,0));
			}
		}
...
function HitByTouching ( pawn Other )
{
	if (Other != None)
	{
		...
		PlayHit(TouchedDamage,vect(0,0,0),'Touched',vect(0,0,0));
		}
...
function Died(pawn Killer, name damageType, vector HitLocation)
{
	...
	if (IsNPC != true) // Patch change - This only works if the Pawn is not an NPC
	{
		if(JazzThinker(Self) == None)
		{
			...
			// Trigger when dead.  (Do here so we don't risk retriggering as Unique enemy)
			//
			if (DeathTriggerTagOnce != '')
			{
				foreach AllActors( class 'Actor', A, DeathTriggerTagOnce )
					A.Trigger( Killer, Self );
			}
		
			// Store unique trigger
			//
			if (UniqueEnemy)
				if (JazzPlayer(Killer) != None)
					JazzPlayer(Killer).AddEventDone(DeathTriggerTag);
			...
			If (DeathEffect != None)
			{
				DeathEffectActor = spawn(class<actor>(DeathEffect));
				DeathEffectActor.SetRotation(Rotation);
			}
...
// Patch change - this is used for Jump-On deaths
function JumpedOnDied(pawn Killer, name damageType, vector HitLocation)
{
	local Actor A;
	local Actor DeathEffectActor;
	...
	if (IsNPC != true) // Patch change - This only works if the Pawn is not an NPC
	{
		if(JazzThinker(Self) == None)
		{
			...
			If (DeathEffect != None)
			{
				DeathEffectActor = spawn(class<actor>(JumpedOnDeathEffect));
				DeathEffectActor.SetRotation(Rotation);
			}
