...
// Patch change - the music volume no longer changes when you enter or exit the main title screen.
// So this has to be removed in order to prevent wierd audio volume gliches.
...
function bool ProcessRight()
{
	if ( Selection == 1 )
	{
		MusicVol = Min(255, MusicVol + 4);
		PlayerOwner.ConsoleCommand("set ini:Engine.Engine.AudioDevice MusicVolume "$MusicVol);
		return true;
	}
	else if ( Selection == 2 )
	{
		SoundVol = Min(255, SoundVol + 4);
		PlayerOwner.ConsoleCommand("set ini:Engine.Engine.AudioDevice SoundVolume "$SoundVol);
		return true;
	}
	else if ( Selection == 3 )
