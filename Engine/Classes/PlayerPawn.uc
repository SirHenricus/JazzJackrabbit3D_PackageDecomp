...
var bool		bGodMode; 		// Patch change - This wasn't here before!?
var bool		bNoclip;  		// Patch change - This one either, need some cheats in Jazz3!
//var Float		PreviousPhysics;// Patch change - Need this to store the previous physics for Noclip. 
...
var bool		bUseJoystick; // Patch change - this is needed for changing the camera controls since they don't work well with mouse controls
...
)
{
	...
	ViewRotation.Pitch = 0;//ViewPitch; // Patch change - testing to see if this works online.
...
function UpdateRotation(float DeltaTime, float maxPitch)
{
	...
	// Patch change - this stuff is used for cooperating with the new character code.
	...
	// Patch change - This code is altered in order to cooperate with the new character code.
	
	newRotation = Rotation;
	if (!IsInState('Frozen'))
	{
		if ((Physics != PHYS_Falling) && (sqrt(abs(aForward*aForward+aStrafe*aStrafe))!=0) )
...
ignores SeePlayer, HearNoise, Bump;
	...
	function ProcessMove(float DeltaTime, vector NewAccel, eDodgeDir DodgeMove, rotator DeltaRot)	
	{
		...
		if ( bUpAndOut && !Region.Zone.bWaterZone && bNoclip && CheckWaterJump(Temp) ) //check for waterjump
		{
			velocity.Z = 330 + 2 * CollisionRadius; //set here so physics uses this for remainder of tick
			PlayDuck();
			GotoState('PlayerWalking');
...
defaultproperties
{
     ...
     WeaponPriority(2)=JackrabbitGun
     ...
     WeaponPriority(8)=ut_biorifle
     WeaponPriority(9)=PulseGun
     WeaponPriority(10)=SniperRifle
     WeaponPriority(11)=ripper
     WeaponPriority(12)=minigun2
